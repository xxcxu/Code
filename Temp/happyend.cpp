...
namespace Codespace {
    ...
    function Solver(int test_case = 0) -> void {
        read(n);
        for (int i = 1; i <= n; ++i) read(h[i], c[i]);
        ...
        Dfs(1, 0);
        int max = 0, id = 1;
        for (int i = 1; i <= n; ++i) if (dep[i] > max) max = dep[i], id = i;
        int ans = -h[id];
        for (int i = 1; i <= n; ++i) ans += c[id] * (max - dep[i]) + c[id] * c[id];
        return write(ans, '\n');
    }
}
function main(int argv, char *argc[]) -> decltype(0) {
    ...
    #endif
    int T, Case = 0;
    if (isfreopen) freopenI("happyend"), freopenO("happyend");
