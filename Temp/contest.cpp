...
namespace Codespace {
    ...
    int n, a, b, ans;
    int f[N], g[N], fac[N];
    int inv[N], inv2[N], finv[N];
    int p[N], q[N], pq[N], ipq[N];
    ...
    function C(int n, int m) -> int {return Mul(fac[n], finv[m], finv[n - m]);}
    function qpow(int x, int y) -> int {
        int ans = 1;
        for (; y; y >>= 1, x = Mul(x, x))
    ...
    function Solver(int test_case = 0) -> void {
        read(n, a, b); ans = 0; Init();
        p[0] = q[0] = g[1] = inv2[0] = f[0] = 1;
        p[1] = Mul(a, qpow(b, P - 2));
        q[1] = P + 1 - p[1];
        for (int i = 2; i <= n; ++i) p[i] = Mul(p[i - 1], p[1]);
        for (int i = 2; i <= n; ++i) q[i] = Mul(q[i - 1], q[1]);
        for (int i = 2; i <= n; ++i) g[i] = Inc(Mul(g[i - 1], g[i - 1]), 2);
        if (p[1] == q[1]) {
            for (int i = 1; i < n; ++i)
                ans = Inc(ans, Mul(C(n, i), qpow(qpow(2, 1ll * i * (n - i) % (P - 1)), P - 2), g[i]));
        } else {
            for (int i = 0; i <= n; ++i) pq[i] = Inc(p[i], P - q[i]);
            for (int i = 0; i <= n; ++i) ipq[i] = qpow(pq[i], P - 2);
            for (int i = 1; i < n; ++i) {
                int _inv = ipq[i], _red = pq[n - i + 1];
                f[i] = Mul(f[i - 1], _inv, _red);
                ans = Inc(ans, Mul(f[i], g[i]));
            }
        } write(ans, '\n');
    }
}
function main(int argv, char *argc[]) -> decltype(0) {
    ...
    #endif
    int T, Case = 0;
    if (isfreopen) freopenI("contest"), freopenO("contest");
